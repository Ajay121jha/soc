...
const techCategories = [
    ...
    {
        name: "Security",
        subcategories: ["Firewall"]
    },
    {
        name: "Data Center",
        subcategories: ["O.S", "Database", "Application", "Storage"],
    },
    { name: "Threat Intelligence Advisory", subcategories: [] },
    {
        name: "Cloud",
        subcategories: ["AWS", "Azure", "GCP", "Oracle"],
    },
];
...
This is an automated notification from the Advisory System.
  ...
  const handleCategoryChange = (e) => {
    ...
    const category = techCategories.find(c => c.name === categoryName);
    if (category && category.subcategories.length > 0) {
        ...
        setSubCategories([]);
        setSelectedSubCategory("");
        const matchingTech = techStacks.find(t => t.name === categoryName);
        setNewAdvisory(prev => ({ ...prev, techStackId: matchingTech ? matchingTech.id : "" }));
    }
  };
  ...
  const handleSubCategoryChange = (e) => {
      ...
      const matchingTech = techStacks.find(t => t.name === subCategoryName);
      setNewAdvisory(prev => ({ ...prev, techStackId: matchingTech ? matchingTech.id : "" }));
  };

  const handleConfigCategoryChange = (e) => {
    const categoryName = e.target.value;
    setSelectedConfigCategory(categoryName);
    const category = techCategories.find(c => c.name === categoryName);
    if (category && category.subcategories.length > 0) {
        setConfigSubCategories(category.subcategories);
        setNewTechStackId("");
        setSelectedConfigSubCategory("");
    } else {
        setConfigSubCategories([]);
        setSelectedConfigSubCategory("");
        const matchingTech = techStacks.find(ts => ts.name === categoryName);
        setNewTechStackId(matchingTech ? matchingTech.id : "");
    }
  };

  const handleConfigSubCategoryChange = (e) => {
    const subCategoryName = e.target.value;
    setSelectedConfigSubCategory(subCategoryName);
    const matchingTech = techStacks.find(ts => ts.name === subCategoryName);
    setNewTechStackId(matchingTech ? matchingTech.id : "");
  };
              ...
              <div className="assign-tech-form">
                ...
                {configSubCategories.length > 0 && (
                    <div className="form-group">
                        ...
                        >
                            <option value="">-- Select Sub-Category --</option>
                            {configSubCategories.map((subCat) => (
                                <option key={subCat} value={subCat}>
                                    {subCat}
                                </option>
                            ))}
